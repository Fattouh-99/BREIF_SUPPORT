...
export const MacbookScroll = ({
  ...
  const springConfig = {
    ...
    damping: 15,
    mass: 0.5,
    restDelta: 0.001 // Added for smoother end of animation
  };

  // Adjusted transform ranges for better scroll effect
  const scaleX = useSpring(
    useTransform(
      scrollYProgress,
      [0, 0.4], // Extended range
      [1.2, isMobile ? 1 : 1.5]
    ),
    springConfig
  );
  ...
  const translate = useSpring(
    useTransform(
      scrollYProgress,
      [0, 1],
      [0, 1600] // Increased translation distance
    ),
    springConfig
  );
  ...
  const rotate = useSpring(
    useTransform(
      scrollYProgress,
      [0.1, 0.15, 0.4], // Adjusted rotation timing
      [-28, -28, 0]
    ),
    springConfig
  );
  ...
  return (
    <div
      ref={ref}
      className="min-h-[250vh] flex flex-col items-center py-0 md:py-96 justify-start flex-shrink-0 [perspective:800px] transform md:scale-100 scale-[0.35] sm:scale-50"
    >
      <Lid
        src={src}
        scaleX={scaleX}
      ...
      />
      {/* Base area */}
      <div className="h-[22rem] w-[32rem] bg-gray-400 dark:bg-[#272729] rounded-2xl overflow-hidden relative -z-10">
        {/* above keyboard bar */}
